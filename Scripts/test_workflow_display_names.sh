...
# 说明：DEFAULT 的外层显示名继续体现设备、防火墙栈和 overlays；
...
if printf '%s\n' "$default_line" | grep -q 'inputs.WRT_SOURCE_FLAVOR'; then
	echo "DEFAULT workflow name should no longer include source flavor" >&2
	exit 1
fi
