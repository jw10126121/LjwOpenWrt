...
assert_contains 'CCACHE_DIR="${{ env.OPENWRT_PATH }}/.ccache" ccache -s > "$RUNNER_TEMP/ccache-before.txt" || true' "workflow should persist pre-build ccache stats for later summary"
assert_contains 'CCACHE_DIR="${{ env.OPENWRT_PATH }}/.ccache" ccache -s > "$RUNNER_TEMP/ccache-after.txt" || true' "workflow should persist post-build ccache stats for later summary"
